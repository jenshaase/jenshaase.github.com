<template>
  <div class="module container">
    ...
    <div class="row">
        <div class="col">
            <div class="map" ref="map" v-bind:style="{ 'height': mapHeightInPx }"></div>
        </div>
    </div>
    <div class="stat row">
        <div class="col">
            <div class="stat__element stat__element--distance">
                <div class="stat__element-caption">Distance</div>
                <div class="stat__element-value">{{ distance }} km</div>
            </div>
        </div>
        <div class="col">
            <div class="stat__element stat__element--elevation">
                <div class="stat__element-caption">Elevation Gain</div>
                <div class="stat__element-value">{{ elevationGain }} m</div>
            </div>
        </div>
    </div>
  </div>
...
export default {
    ...
    mounted () {
        ...
        mapbox.accessToken = 'pk.eyJ1IjoiamVuc2hhYXNlIiwiYSI6ImNqZ2lmNjBnODBsaW4zM3FxZnptZjh0Nm0ifQ.nwSisV89eFZOuNYBmds_AQ'
    ...
    methods: {
        getMapBound (mapbox, geojson) {
            if (geojson.features[0].geometry.type === "MultiLineString") {
                var coordinates = [].concat.apply([], geojson.features[0].geometry.coordinates);
