<template>
  <figure class="image-aspect-ratio" :class="{ 'image-aspect-ratio--zoom-on-hover': zoomOnHover }" v-bind:style="ratioPadding" itemscope itemtype="http://schema.org/ImageObject">
    <img v-if="isImageUrl" 
    ...
    <figcaption itemprop="caption description" v-if="title">{{ title }}</figcaption>
  </figure>
</template>
...
export default {
    ...
    computed: {
        ...
        photoswipeImageData () {
            if (this.isImageUrl) {
                ...
                let lastImage = this.src.images[this.src.images.length - 1]
                return {
                    src: lastImage.path,
                    w: lastImage.width,
                    h: lastImage.height,
        ...
        ratioPadding () {
            var ratio = (this.ratioHeight / this.ratioWidth) * 100
            return {
                'padding-top': ratio + '%'
            }
        }
    ...
    mounted () {
        this.photoswipeIndex = this.$photoswipe.register(this.photoswipeGroup, this.photoswipeImageData)
    }
}
</script>
